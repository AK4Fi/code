...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch_num in range(epochs):
        ...
        total_acc_train = 0
        total_loss_train = 0
        all_preds_train = []  # 保存训练集所有预测结果
        all_labels_train = []  # 保存训练集所有真实标签
        # 进度条函数tqdm
...
# def check_class_distribution(data, name):
#     print(f"\n{name} 类别分布:")
#     print(pd.Series(data['Class']).value_counts().sort_index())
#
# check_class_distribution(df_train, "训练集")
# check_class_distribution(df_val, "验证集")
# check_class_distribution(df_test, "测试集")
