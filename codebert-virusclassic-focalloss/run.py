...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        pt = torch.exp(-ce_loss) # 计算 p_t = e^{-CE}，即模型预测正确类别的概率
        focal_loss = self.alpha * (1 - pt) ** self.gamma * ce_loss# Focal Loss 核心公式

        # 添加类别权重 alpha
        if self.alpha is not None:
            alpha = self.alpha.to(inputs.device)  # 确保 alpha 在正确设备上
            alpha = alpha.gather(0, targets)  # 按 target 索引选择对应类别的 alpha
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    criterion = FocalLoss(alpha=0.25, gamma=2)  # 可调整超参数
